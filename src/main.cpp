...
// Copyright (c) 2013 Californium developers
...
const string strMessageMagic = "Californium Signed Message:\n";
...
static const int64 nMinSubsidy = .005 * COIN;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        if (TestNet())
        {
            ...
            // If the new block's timestamp is more than 2* 90 seconds
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CaliforniumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CaliforniumMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CaliforniumMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CaliforniumMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CaliforniumMiner terminated\n");
        throw;
    }
}
